function BER = MIMO_SC(SNR_dB,symbol_num, bit_seq, H, N, bit_symbol)
%% generate signal
n_all = sqrt(1/2) * (randn(symbol_num,2,2) + 1i*randn(symbol_num,2,2));
SNR = power(10,SNR_dB/10);
Y = QPSK_mod_SC(bit_seq, symbol_num, H, N, n_all, SNR);
...
for i = 1:symbol_num
    ...
    d = zeros(1,M);
    for j = 1:M
        x1 = QPSK_map(ps(1,1,j), ps(1,2,j));
        x2 = QPSK_map(ps(2,1,j), ps(2,2,j));
        x_ = [x1, -conj(x2); x2, conj(x1)];
        d(j) = power(norm(y - sqrt(SNR / N) * H * x_, 'fro'), 2);
    end
    r = ps(:,:,d==min(d));
